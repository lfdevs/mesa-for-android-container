...
VkResult enumerate_physical_device(struct vk_instance *_instance)
{
   ...
   if (actual_count > 0) {
      physical_devices = malloc(actual_count * sizeof(VkPhysicalDevice));
      if (!physical_devices)
   ...
   if (result == VK_INCOMPLETE) {
      ...
      VkPhysicalDevice *larger_array = realloc(physical_devices,
                                                larger_count * sizeof(VkPhysicalDevice));
      if (!larger_array) {
                                    ...
                                    VkPhysicalDeviceProperties *pProperties)
{
   ...
   const char *device_name_env = getenv("WRAPPER_DEVICE_NAME");
   if (device_name_env)
                                     ...
                                     VkPhysicalDeviceProperties2* pProperties)
{
   ...
   if (device_name_env)
      snprintf(pProperties->properties.deviceName,
               sizeof(pProperties->properties.deviceName), "%s",
               device_name_env);
